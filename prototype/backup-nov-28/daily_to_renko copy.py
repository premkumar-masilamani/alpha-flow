...
def calculate_brick_size(df: pd.DataFrame, period_count: int) -> float:
    ...
    for idx, row in lookback_df.iterrows():
        ...
        print(f"  {date_str}: High = {high:.2f}, Low = {low:.2f}, Range = {rng:.2f}")
...
def generate_renko_chart_data(df: pd.DataFrame, period_count: int) -> pd.DataFrame:
    ...
    # Track contributing days for buyer_capital_ratio & buyer_volume_ratio averaging
    contributing_days = []  # List of (date, buyer_capital_ratio, buyer_volume_ratio) tuples
    ...
    for idx, row in df.iterrows():
        ...
        buyer_volume_ratio = row.get("buyer_volume_ratio", None)

        # Add current day to contributing days
        contributing_days.append(
            (current_date, buyer_capital_ratio, buyer_volume_ratio)
        )
        ...
        if bricks_generated_this_iteration:
            ...
            valid_capital_ratios = [
                buyer_capital_ratio
                for _, buyer_capital_ratio, _ in contributing_days
                if buyer_capital_ratio is not None and not pd.isna(buyer_capital_ratio)
            ]
            avg_buyer_capital_ratio = (
                sum(valid_capital_ratios) / len(valid_capital_ratios)
                if valid_capital_ratios
                else None
            )

            valid_volume_ratios = [
                buyer_volume_ratio
                for _, _, buyer_volume_ratio in contributing_days
                if buyer_volume_ratio is not None and not pd.isna(buyer_volume_ratio)
            ]
            avg_buyer_volume_ratio = (
                sum(valid_volume_ratios) / len(valid_volume_ratios)
            ...
            for brick in bricks_generated_this_iteration:
                brick["buyer_capital_ratio"] = avg_buyer_capital_ratio
                brick["buyer_volume_ratio"] = avg_buyer_volume_ratio
...
def main():
    ...
    parser.add_argument(
        "output_file", type=str, help="Path to output CSV file for Renko data"
    )
    parser.add_argument(
        "--period-count",
        type=int,
