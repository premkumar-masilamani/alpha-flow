...
describe('API Service Layer Tests', () => {
    ...
    describe('getTechnicalAnalysis', () => {
        it('should fetch today snapshot aggregating candles and indicators', async () => {
            ...
            mockedAxios.get.mockImplementation((url, config: unknown) => {
                const params = (config as { params?: { timeframe?: string } })?.params;
                if (url.includes('/data')) return Promise.resolve({ data: mockCandles });
                if (url.includes('/indicators') && params?.timeframe === 'daily') return Promise.resolve({ data: mockDaily });
                if (url.includes('/indicators') && params?.timeframe === 'weekly') return Promise.resolve({ data: mockWeekly });
