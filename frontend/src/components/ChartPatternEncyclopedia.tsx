...
import {
    ...
    type ChartPatternDetail,
} from '../config/chartPatterns';

export interface ChartPatternEncyclopediaProps {
...
const ChartPatternEncyclopedia: React.FC<ChartPatternEncyclopediaProps> = ({
    initialPatternId,
}) => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            const targetElement = document.getElementById(`chart-pattern-${initialPatternId}`);
            if (targetElement) {
    ...
    const groupCounts = useMemo(() => {
        ...
        for (const pattern of CHART_PATTERNS) {
            let matches = true;
            if (query) {
                const searchableText = `${pattern.title} ${pattern.id} ${pattern.patternType} ${pattern.shortName} ${pattern.groupName} ${pattern.structure} ${pattern.psychology} ${pattern.outcome} ${pattern.targetRule}`.toLowerCase();
    ...
    const filteredPatternsByGroup = useMemo(() => {
        ...
        const filtered = CHART_PATTERNS.filter((pattern) => {
        ...
        // Group into sections
        const grouped: Record<string, ChartPatternDetail[]> = {
            'bullish-reversals': [],
            'bearish-reversals': [],
            ...
            <div className="shrink-0 bg-slate-900/90 border-b border-slate-800 px-4 md:px-6 py-4 backdrop-blur z-20 space-y-3">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <div className="relative flex-1">
                        ...
                        />
                        <input
                            id="chart-pattern-search"
                            type="text"
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            placeholder="Search chart patterns by name, structure, psychology, or target rule..."
                    ...
                    <div className="shrink-0 flex items-center justify-between sm:justify-end gap-2 text-xs font-mono text-slate-400 bg-slate-950 px-3 py-2 rounded-lg border border-slate-800">
                        ...
                        <span>of {CHART_PATTERNS.length} patterns</span>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5" role="tablist" aria-label="Chart pattern category filters">
                        <button
                            role="tab"
                ...
                {totalFilteredCount === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 text-center space-y-4">
                        ...
                        <div className="space-y-1">
                            <h3 className="text-base font-bold text-slate-200">No matching chart patterns found</h3>
                    ...
                    CHART_PATTERN_GROUPS.map((group) => {
                                    ...
                                    <table className="w-full text-left border-collapse text-sm">
                                        ...
                                        <tbody className="divide-y divide-slate-800/60">
                                            {patterns.map((pattern) => {
                                                const isHighlighted =
                                                    highlightedId === pattern.id ||
                                                    highlightedId === pattern.patternType;
                                                return (
                                                    <tr
                                                        key={pattern.id}
                                                        id={`chart-pattern-${pattern.id}`}
                                                        ...
                                                        <td className="p-4 align-top">
                                                            <div className="space-y-2.5">
                                                                ...
                                                                <div className="flex flex-wrap items-center gap-2">
                                                                    ...
                                                                    <span className="bg-slate-800 text-slate-400 border border-slate-700 font-mono text-[10px] px-2 py-0.5 rounded font-medium capitalize">
                                                                        {pattern.category}
                                                                    </span>
                                                                </div>
                                                                ...
                                                                <div className="text-xs text-slate-300 space-y-1.5 leading-relaxed">
                                                                    ...
                                                                    <div>
                                                                        <span className="font-semibold text-sky-400 mr-1.5">Target &amp; Stop Loss:</span>
                                                                        <span className="text-slate-300">{pattern.targetRule}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </td>
...
export default ChartPatternEncyclopedia;
