...
interface ChartPatternModalProps {
    isOpen: boolean;
    onClose: () => void;
    initialPatternId?: string | null;
}

const ChartPatternModal: React.FC<ChartPatternModalProps> = ({
    isOpen,
    onClose,
    initialPatternId,
}) => {
            ...
            >
                <div className="flex items-center justify-between px-5 py-3.5 bg-slate-950 border-b border-slate-800 select-none">
                    <div className="flex items-center gap-2.5">
                        ...
                        <h2 id="cp-modal-title" className="text-sm md:text-base font-bold text-white tracking-tight">
                ...
                <div className="flex-1 w-full h-full bg-slate-950 overflow-hidden relative flex flex-col min-h-0">
                    <ChartPatternEncyclopedia initialPatternId={initialPatternId} />
                </div>
            </div>
        </div>
    );
...
export default ChartPatternModal;
