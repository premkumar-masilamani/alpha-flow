 ...
 * resume-from-state
 *
 * <p>is deterministic and bit-exact: the running state persisted in {@code
 * indicator_state.internals}
 *
 * <p>carries full internal precision (string-encoded), while values published to {@code
 * indicator_values}
 *
 * <p>are rounded to {@link #PUBLISHED_SCALE} to match the {@code numeric(18,4)} column. Using the
 * same
 *
 * <p>scale and rounding everywhere is what guarantees a resumed run reproduces a full recompute
 * exactly.
 */
