 ...
 *
 * <p>values, so it carries no running state ({@code newStateJson == null}) and is reconstructed on
 *
 * <p>resume purely from the price bars the caller loads. The running sum uses exact {@code
 * BigDecimal}
 *
 * <p>add/subtract (no rounding), so there is no drift; only the final division rounds.
 */
@Component
