 ...
 *
 * <p>This lives in its own bean (rather than as a method on {@link WeeklyPriceCalculator}) so that
 * the {@link Transactional} boundary is honored: a self-invocation from within the same bean would
 * bypass Spring's proxy and silently disable the transaction.
 */
@Component
