 ...
 * Transactional} boundary is honored — a self-invocation would bypass Spring's proxy. The whole
 * ticker is one transaction: its published values and checkpoints commit atomically, so they can
 * never diverge.
 *
 ...
 *       after it; otherwise (cold start, new indicator, or a contiguity gap) recompute from
 *       history.
 *   <li><b>Publish values</b> for the recompute window (resume: bars after the checkpoint,
 *       including the in-progress last bar; backfill: the whole series) by deleting that date range
 *       and reinserting the freshly computed plots.
 ...
 *       never frozen into resume state.
 * </ol>
 *
