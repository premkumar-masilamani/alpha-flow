 ...
 *
 * <p>Lives in its own bean (not as a method on {@link IndicatorCalculator}) so the {@link
 * Transactional}
 *
 * <p>boundary is honored — a self-invocation would bypass Spring's proxy. The whole ticker is one
 *
 * <p>transaction: its published values and checkpoints commit atomically, so they can never
 * diverge.
 *
 ...
 *       <p>the bars, and it is resumable (recursive with internals, or windowed), resume from the
 *       bar
 *       <p>after it; otherwise (cold start, new indicator, or a contiguity gap) recompute from
 *       history.
 *   <li><b>Publish values</b> for the recompute window (resume: bars after the checkpoint,
 *       including
 *       <p>the in-progress last bar; backfill: the whole series) by deleting that date range and
 *       <p>reinserting the freshly computed plots.
 *   <li><b>Checkpoint</b> the running state as of the <i>second-to-last</i> bar — never the most
 *       <p>recent one — so the in-progress bar (e.g. the current week) is republished every run but
...
public class IndicatorTickerProcessor {
  ...
  private void processTimeframe(Ticker ticker, Timeframe timeframe, List<PriceBar> bars) {
    ...
    for (IndicatorState state :
        indicatorStateRepository.findByTickerAndTimeframe(ticker, timeframe)) {

      stateByCombo.put(
          comboKey(state.getIndicatorType(), state.getSource(), state.getParams()), state);
    }
  ...
  private void processCombo(
    ...
    if (resume) {
      ...
      computed =
          windowed
              ...
              : indicator
                  .compute(bars.subList(checkpointIndex + 1, n), seedJson, params, source)
                  .values();
    ...
    indicatorValueRepository.deleteCombo(
        ticker, timeframe, type, source, paramsCanonical, rewriteFrom);
    ...
    if (windowed) {
      ...
      // bars.subList(ci+1, n-1) excludes the in-progress bar; empty when ci == n-2 (state
      // unchanged).

      checkpointInternals =
          indicator
              .compute(bars.subList(checkpointIndex + 1, n - 1), seedJson, params, source)
              .newStateJson();

    } else {

      checkpointInternals =
          indicator.compute(bars.subList(0, n - 1), null, params, source).newStateJson();
    }
