...
public class WeeklyPriceCalculator {
    ...
    public void computeWeeklyPrices() {
        ...
        for (Ticker ticker : tickers) {
            try {
                ...
                log.error("Failed to compute weekly prices for ticker {}: {}", ticker.getTickerSymbol(), e.getMessage(), e);
            }
        }
    ...
    public void processTicker(Ticker ticker) {
        ...
        for (Map.Entry<LocalDate, List<DailyPrice>> entry : dailyPricesByWeek.entrySet()) {
            ...
            // Look up existing weekly price record to update or insert
            WeeklyPrice weeklyPrice = weeklyPriceRepository.findByTickerAndPriceDate(ticker, weekMonday)
                    .orElse(WeeklyPrice.builder()
                            .ticker(ticker)
