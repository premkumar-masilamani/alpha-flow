 ...
 * gets stored in the {@code params} column and forms part of an indicator's natural key. Keys are
 * sorted so the same logical configuration always serializes to the exact same string regardless of
 * how the source map was built (e.g. config-binding order) — this keeps the natural key stable
 * across runs. Lookups are by name, so ordering never affects computation.
 */
