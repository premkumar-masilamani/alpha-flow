 ...
 * equals their simple average (SMA); thereafter {@code ema = value·k + emaPrev·(1−k)} with {@code k
 * = 2/(period+1)}. All arithmetic runs at {@link IndicatorMath#INTERNAL_SCALE}.
 *
 * <p>Construct {@link #fresh(int)} for a backfill (self-seeds), or {@link #seeded(int, BigDecimal)}
 * to resume from a persisted EMA value (already past seeding) — the two paths produce identical
 * values for any bar once both are seeded, which is what makes resume bit-exact. Reused by both
 * {@link EmaIndicator} and {@link MacdIndicator} (whose three chained EMAs are each an
 * accumulator).
 */
