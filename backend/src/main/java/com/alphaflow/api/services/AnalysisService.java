...
public class AnalysisService {
    ...
    public AnalysisService(DailyPriceService dailyPriceService,
    ...
    public void computeAnalysis() {
        ...
        List<Ticker> activeTickers = tickerRepository.findAll().stream()
    ...
    public AnalysisResult computeAndPersist(Ticker ticker) {
        ...
        // Fetch Indicators
        List<IndicatorSeriesDTO> dailyIndicators = indicatorService.getIndicatorSeries(symbol, Timeframe.DAILY, 0, 50);
        List<IndicatorSeriesDTO> weeklyIndicators = indicatorService.getIndicatorSeries(symbol, Timeframe.WEEKLY, 0, 10);

        // Date reference for this analysis
        LocalDate priceDate = dailyCandles.isEmpty() ? LocalDate.now() : dailyCandles.getLast().priceDate();
        ...
        // Overall consensus
        boolean doubleScreenBuy = "BUY".equals(macd.signal) && "BUY".equals(stoch.signal) && "BUY".equals(rsi.signal);
        boolean doubleScreenSell = "SELL".equals(macd.signal) && "SELL".equals(stoch.signal) && "SELL".equals(rsi.signal);
        ...
        AnalysisResult result = analysisResultRepository.findByTicker(ticker)
    ...
    private CalculatedSignal evaluateWeeklyMacd(List<IndicatorSeriesDTO> weeklyIndicators) {
        Optional<IndicatorSeriesDTO> macdSeriesOpt = weeklyIndicators.stream()
    ...
    private CalculatedSignal evaluateDailyStochastic(List<IndicatorSeriesDTO> dailyIndicators) {
        Optional<IndicatorSeriesDTO> stochSeriesOpt = dailyIndicators.stream()
    ...
    private CalculatedSignal evaluateDailyRsi(List<IndicatorSeriesDTO> dailyIndicators) {
        Optional<IndicatorSeriesDTO> rsiSeriesOpt = dailyIndicators.stream()
        ...
        if (rsi0.compareTo(rsi1) > 0) {
            return new CalculatedSignal("BUY", "Uptick (RSI: " + rsi0.setScale(1, RoundingMode.HALF_UP) + ")");
        } else if (rsi0.compareTo(rsi1) < 0) {
            return new CalculatedSignal("SELL", "Downtick (RSI: " + rsi0.setScale(1, RoundingMode.HALF_UP) + ")");
        } else {
            return new CalculatedSignal("HOLD", "Flat (RSI: " + rsi0.setScale(1, RoundingMode.HALF_UP) + ")");
        }
    }

    private CalculatedSignal evaluateVolume(List<OhlcvDTO> dailyCandles, List<IndicatorSeriesDTO> dailyIndicators) {
        ...
        Optional<IndicatorSeriesDTO> volSmaOpt = dailyIndicators.stream()
        ...
        if (isHeavyVolume) {
            ...
            String val = isGreen ? "Green Candle with Normal Volume" : (isRed ? "Red Candle with Normal Volume" : "Doji with Normal Volume");
            return new CalculatedSignal("HOLD", val);
        }
    }

    private CalculatedSignal evaluateEma(List<IndicatorSeriesDTO> dailyIndicators) {
        Optional<IndicatorSeriesDTO> ema5Opt = dailyIndicators.stream()
                .filter(s -> "EMA".equalsIgnoreCase(s.type()) && s.params().contains("period=5"))
                .findFirst();
        Optional<IndicatorSeriesDTO> ema13Opt = dailyIndicators.stream()
                .filter(s -> "EMA".equalsIgnoreCase(s.type()) && s.params().contains("period=13"))
                .findFirst();
        Optional<IndicatorSeriesDTO> ema26Opt = dailyIndicators.stream()
                .filter(s -> "EMA".equalsIgnoreCase(s.type()) && s.params().contains("period=26"))
                .findFirst();

        if (ema5Opt.isEmpty() || ema13Opt.isEmpty() || ema26Opt.isEmpty() ||
                ema5Opt.get().points().size() < 2 || ema13Opt.get().points().size() < 2 || ema26Opt.get().points().size() < 2) {
        ...
        if (e5_0 == null || e5_1 == null || e13_0 == null || e13_1 == null || e26_0 == null || e26_1 == null) {
        ...
        if (pco13 || pco26) {
            String value = pco13 && pco26 ? "5 EMA Positive Crossover with 13 & 26 EMA" :
                    (pco13 ? "5 EMA Positive Crossover with 13 EMA" : "5 EMA Positive Crossover with 26 EMA");
            return new CalculatedSignal("BUY", value);
        } else if (nco13 || nco26) {
            String value = nco13 && nco26 ? "5 EMA Negative Crossover with 13 & 26 EMA" :
                    (nco13 ? "5 EMA Negative Crossover with 13 EMA" : "5 EMA Negative Crossover with 26 EMA");
            return new CalculatedSignal("SELL", value);
