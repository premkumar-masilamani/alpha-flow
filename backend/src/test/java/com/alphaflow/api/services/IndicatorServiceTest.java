...
class IndicatorServiceTest {
  ...
  void setUp() {
    ...
    service = new IndicatorService(indicatorProperties, new ApiProperties(), tickerRepository,

        dailyPriceRepository, weeklyPriceRepository, indicatorValueRepository);
  ...
  void discoveryFlattensConfiguredMatrix() {
    ...
    indicatorProperties.setTimeframes(Map.of(

        Timeframe.DAILY, List.of(def(IndicatorType.EMA, PriceSource.CLOSE, Map.of("period", 5))),

        Timeframe.WEEKLY, List.of(def(IndicatorType.MACD, PriceSource.CLOSE, Map.of("fast", 12, "slow", 26, "signal", 9)))));
    ...
    IndicatorConfigDTO ema = configs.stream().filter(c -> c.type().equals("EMA")).findFirst().orElseThrow();
    ...
    IndicatorConfigDTO macd = configs.stream().filter(c -> c.type().equals("MACD")).findFirst().orElseThrow();
  ...
  void seriesGroupsByComboAndDate() {
    ...
    IndicatorSeriesDTO ema = series.stream().filter(s -> s.type().equals("EMA")).findFirst().orElseThrow();
    ...
    IndicatorSeriesDTO macd = series.stream().filter(s -> s.type().equals("MACD")).findFirst().orElseThrow();
