...
class YahooFinanceDownloaderTest {
    ...
    void testDownloadUpToDateNoAction() {
        ...
        when(dailyRepo.findLatestPriceDatesForActiveTickers()).thenReturn(
            latestDates
        );
    ...
    void testDownloadValidParseAndSave() {
        ...
        config.setDownloadUrl(
            jsonUrl.toString() + "?symbol={symbol}&start={start}&end={end}"
        );
    ...
    void testDownloadConnectionExceptionHandled() {
        ...
        config.setDownloadUrl(
            "invalidproto://foo?symbol={symbol}&start={start}&end={end}"
        );
