...
class IndicatorAAPLTest {
    ...
    void testAaplSMA() {
        IndicatorResult r = new SmaIndicator().compute(
                aaplBars, null, IndicatorParams.parse("period=20"), PriceSource.CLOSE);
        assertFalse(r.values().isEmpty());
        PlotPoint latest = r.values().get(r.values().size() - 1);
        System.out.println("[AAPL-SMA-20] Latest point: Date=" + latest.date() + ", Value=" + latest.value());
    ...
    void testAaplEMA() {
        ...
        assertFalse(r.values().isEmpty());
        PlotPoint latest = r.values().get(r.values().size() - 1);
        System.out.println("[AAPL-EMA-20] Latest point: Date=" + latest.date() + ", Value=" + latest.value());
        assertEquals(LocalDate.of(2026, 5, 29), latest.date());
    ...
    void testAaplRSI() {
        IndicatorResult r = new RsiIndicator().compute(
                aaplBars, null, IndicatorParams.parse("period=14"), PriceSource.CLOSE);
        assertFalse(r.values().isEmpty());
        PlotPoint latest = r.values().get(r.values().size() - 1);
        System.out.println("[AAPL-RSI-14] Latest point: Date=" + latest.date() + ", Value=" + latest.value());
        assertEquals(LocalDate.of(2026, 5, 29), latest.date());
    ...
    void testAaplMACD() {
        IndicatorResult r = new MacdIndicator().compute(
                aaplBars, null, IndicatorParams.parse("fast=12,slow=26,signal=9"), PriceSource.CLOSE);
        assertFalse(r.values().isEmpty());
        ...
        System.out.println("[AAPL-MACD] Latest point: MACD=" + macd.value() + ", Signal=" + signal.value() + ", Hist=" + hist.value());
    ...
    void testAaplStochastic() {
        IndicatorResult r = new StochasticIndicator().compute(
                aaplBars, null, IndicatorParams.parse("k=14,kSmooth=3,dSmooth=3"), PriceSource.CLOSE);
        assertFalse(r.values().isEmpty());
        ...
        System.out.println("[AAPL-STOCHASTIC] Latest point: %K=" + kVal.value() + ", %D=" + dVal.value());
