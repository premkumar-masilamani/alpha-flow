...
/**
 * Verifies the {@link IndicatorTickerProcessor} orchestration — the riskiest new logic — with mocked
 * repositories: cold-start backfill writes the whole series and checkpoints the second-to-last bar;
 * steady-state resume rewrites only the in-progress last bar and leaves the checkpoint where it was.
 */
class IndicatorTickerProcessorTest {
    ...
    private static List<DailyPrice> dailyBars() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            bars.add(DailyPrice.builder()
                    .priceDate(date(i))
                    .priceOpen(close).priceHigh(close.add(BigDecimal.ONE)).priceLow(close.subtract(BigDecimal.ONE))
                    .priceClose(close)
                    .volume(100L + i)
                    .build());
        }
    ...
    void setUp() {
        ...
        IndicatorRegistry registry = new IndicatorRegistry(List.of(
                new EmaIndicator(), new SmaIndicator(), new RsiIndicator(),
        ...
        processor = new IndicatorTickerProcessor(registry, properties, dailyRepo, weeklyRepo, valueRepo, stateRepo);

        ticker = Ticker.builder().tickerId(1L).tickerSymbol("TEST").tickerName("Test").isActive(true).build();
    ...
    void coldStartBackfillsWholeSeriesAndCheckpointsSecondToLastBar() {
        ...
        verify(valueRepo).deleteCombo(ticker, Timeframe.DAILY, IndicatorType.SMA, PriceSource.VOLUME, "period=2", EPOCH);
        ...
        IndicatorState emaState = states.stream().filter(s -> s.getIndicatorType() == IndicatorType.EMA).findFirst().orElseThrow();
        IndicatorState smaState = states.stream().filter(s -> s.getIndicatorType() == IndicatorType.SMA).findFirst().orElseThrow();
    ...
    void steadyStateResumeRewritesOnlyInProgressBar() {
        ...
        String internalsAtSecondToLast = new EmaIndicator()
                .compute(bars.subList(0, N - 1), null, IndicatorParams.parse("period=3"), PriceSource.CLOSE)
                .newStateJson();
        ...
        IndicatorState priorEma = IndicatorState.builder()
                ...
                .lastPriceDate(date(N - 2)).internals(internalsAtSecondToLast)
                .build();
        ...
        verify(valueRepo).deleteCombo(ticker, Timeframe.DAILY, IndicatorType.EMA, PriceSource.CLOSE, "period=3", date(N - 1));
        ...
        BigDecimal fullValue = new EmaIndicator()
                .compute(bars, null, IndicatorParams.parse("period=3"), PriceSource.CLOSE)
                .values().stream().filter(p -> p.date().equals(date(N - 1))).findFirst().orElseThrow().value();
        assertEquals(fullValue, written.getFirst().getValue());

        // Checkpoint does not regress (stays at the second-to-last bar).
        IndicatorState saved = capturedStates().stream()
                .filter(s -> s.getIndicatorType() == IndicatorType.EMA).findFirst().orElseThrow();
    ...
    void warmupProducesNoCheckpointForRecursiveIndicator() {
        ...
        boolean emaCheckpointed = capturedStates().stream().anyMatch(s -> s.getIndicatorType() == IndicatorType.EMA);
